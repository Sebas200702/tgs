...
import { ArrowUpRight, CheckCircle2, Search, Clock, Check, X } from "lucide-react";
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <Tabs defaultValue="registrar">
          <TabsList className="bg-[#E8F3E4] mb-4">
            <TabsTrigger value="registrar" data-testid="tab-registrar">Nueva solicitud</TabsTrigger>
            <TabsTrigger value="consultar" data-testid="tab-consultar">Consultar mi solicitud</TabsTrigger>
          </TabsList>
          <TabsContent value="registrar"><FormRegistro navigate={navigate} /></TabsContent>
          <TabsContent value="consultar"><ConsultaRegistro /></TabsContent>
        </Tabs>
      </div>
    </div>
  );
...
function FormRegistro({ navigate }) {
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    try {
      const r = await api.post("/registros-cancha", { ...form, num_canchas: Number(form.num_canchas) });
      setDone(r.data);
  ...
  if (done) {
    return (
      <div className="bg-white border border-[#E8F3E4] rounded-3xl p-8 text-center" data-testid="registro-success">
        <CheckCircle2 size={64} className="mx-auto text-[#1F4D2A]" />
        <h2 className="font-display text-3xl font-black text-[#1F4D2A] mt-4">¡Solicitud recibida!</h2>
        <p className="text-[#4B5563] mt-3 max-w-xl mx-auto">
          Recibimos tu solicitud para afiliar <strong>{done.nombre_local}</strong>.
          Nuestro equipo revisará tus datos y te contactará en las próximas 24-48 horas al correo <strong>{done.email}</strong>.
        </p>
        <div className="mt-5 bg-[#E8F3E4] rounded-2xl p-4 inline-block">
          <div className="text-xs text-[#1F4D2A]">N° de solicitud</div>
          <div className="font-mono font-bold text-[#1F4D2A]" data-testid="registro-id">{done.id.slice(0, 8).toUpperCase()}</div>
          <div className="text-xs mt-2 inline-flex items-center gap-1 bg-yellow-100 text-yellow-700 px-2 py-1 rounded-full"><Clock size={12} /> Pendiente de revisión</div>
        </div>
        <div className="mt-6 flex gap-3 justify-center flex-wrap">
          <button onClick={() => { setDone(null); setForm({ nombre_local: "", direccion: "", telefono: "", email: "", nombre_contacto: "", num_canchas: 1, mensaje: "" }); }} className="pill-btn-outline text-sm">Enviar otra</button>
          <button onClick={() => navigate("/")} className="pill-btn text-sm" data-testid="back-home-btn">Volver al inicio</button>
        </div>
      </div>
    );
  }
...
function ConsultaRegistro() {
  ...
  const consultar = async (e) => {
    ...
    setLoading(true);
    try {
      const r = await api.get(`/registros-cancha/by-email/${encodeURIComponent(email)}`);
      setItems(r.data);
    } catch { toast.error("Error consultando"); }
    finally { setLoading(false); }
  };

  const estadoBadge = (estado) => {
    if (estado === "aprobado") return <span className="text-xs px-3 py-1 rounded-full bg-[#D4E8C9] text-[#1F4D2A] font-semibold inline-flex items-center gap-1"><Check size={12} /> Aprobado</span>;
    if (estado === "rechazado") return <span className="text-xs px-3 py-1 rounded-full bg-red-100 text-red-700 font-semibold inline-flex items-center gap-1"><X size={12} /> Rechazado</span>;
    return <span className="text-xs px-3 py-1 rounded-full bg-yellow-100 text-yellow-700 font-semibold inline-flex items-center gap-1"><Clock size={12} /> Pendiente</span>;
  };

  return (
    <div className="bg-white border border-[#E8F3E4] rounded-3xl p-6 md:p-8 space-y-4" data-testid="consulta-form">
      <h2 className="font-display text-2xl font-bold text-[#1F4D2A]">Consulta el estado de tu solicitud</h2>
      <form onSubmit={consultar} className="flex gap-3">
        <Input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Email registrado" required className="flex-1" data-testid="consulta-email" />
        <button type="submit" disabled={loading} className="pill-btn text-sm disabled:opacity-50" data-testid="consulta-btn"><Search size={16} /> {loading ? "Buscando..." : "Buscar"}</button>
      </form>
      {items && (
        <div className="space-y-3 mt-3" data-testid="consulta-results">
          {items.length === 0 && <p className="text-sm text-[#4B5563] text-center py-6">No encontramos solicitudes con ese email.</p>}
          {items.map(it => (
            <div key={it.id} className="bg-[#E8F3E4] rounded-2xl p-4">
              <div className="flex justify-between items-start gap-3">
                <div>
                  <h3 className="font-display font-bold text-[#1F4D2A]">{it.nombre_local}</h3>
                  <p className="text-xs text-[#4B5563]">{it.direccion}</p>
                  <p className="text-xs text-[#4B5563] mt-1">Solicitud #{it.id.slice(0, 8).toUpperCase()} · {it.num_canchas} cancha(s)</p>
                </div>
                {estadoBadge(it.estado)}
              </div>
              {it.estado === "aprobado" && <p className="text-xs text-[#1F4D2A] mt-2">¡Tu local ya está en OutSide! Nuestro equipo te contactará para configurar precios y horarios.</p>}
              {it.estado === "rechazado" && <p className="text-xs text-red-700 mt-2">Tu solicitud no pudo ser aprobada. Para más información contáctanos al hola@outside.com.co</p>}
            </div>
          ))}
        </div>
      )}
